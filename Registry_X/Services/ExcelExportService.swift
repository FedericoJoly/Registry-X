...
class ExcelExportService {
    ...
    private static func createRegistrySheet(event: Event) -> String {
        ...
        for (index, header) in headers.enumerated() {
            xml += "<c r=\"\(columnLetter(index))1\" s=\"1\"><v>\(xmlEscape(header))</v></c>"
        }
        // Add product columns
        for (index, product) in allProducts.enumerated() {
            ...
            xml += "<c r=\"\(columnLetter(colIndex))1\" s=\"1\"><v>\(xmlEscape(productHeader))</v></c>"
        }
        xml += "</row>\n"
        ...
        for (txIndex, transaction) in transactions.enumerated() {
            ...
            xml += "<c r=\"\(columnLetter(colNum))\(rowNum)\"><v>\(xmlEscape(txId))</v></c>"
            colNum += 1
            
            // TX Date
            let dateFormatter = DateFormatter()
            dateFormatter.dateFormat = "yyyy-MM-dd"
            xml += "<c r=\"\(columnLetter(colNum))\(rowNum)\"><v>\(dateFormatter.string(from: transaction.timestamp))</v></c>"
            colNum += 1
            ...
            xml += "<c r=\"\(columnLetter(colNum))\(rowNum)\"><v>\(xmlEscape(methodName))</v></c>"
            colNum += 1
            
            // Currency
            xml += "<c r=\"\(columnLetter(colNum))\(rowNum)\"><v>\(transaction.currencyCode)</v></c>"
            colNum += 1
            ...
            // Note
            xml += "<c r=\"\(columnLetter(colNum))\(rowNum)\"><v>\(xmlEscape(transaction.note ?? ""))</v></c>"
            colNum += 1
            
            // Email
            xml += "<c r=\"\(columnLetter(colNum))\(rowNum)\"><v>\(xmlEscape(transaction.receiptEmail ?? ""))</v></c>"
            colNum += 1
    ...
    private static func createProductsSummarySheet(event: Event) -> String {
        ...
        for (index, (_, stats)) in sortedProducts.enumerated() {
            let rowNum = index + 2
            xml += "<row r=\"\(rowNum)\">"
            xml += "<c r=\"A\(rowNum)\"><v>\(xmlEscape(stats.name))</v></c>"
            xml += "<c r=\"B\(rowNum)\"><v>\(xmlEscape(stats.subgroup ?? ""))</v></c>"
        ...
        xml += "<row r=\"\(totalRow)\">"
        xml += "<c r=\"A\(totalRow)\"><v>TOTAL</v></c>"
    ...
    private static func createCurrenciesSheet(event: Event) -> String {
        ...
        for (index, (_, stat)) in sortedStats.enumerated() {
            let rowNum = index + 2
            xml += "<row r=\"\(rowNum)\">"
            xml += "<c r=\"A\(rowNum)\"><v>\(stat.currency)</v></c>"
            xml += "<c r=\"B\(rowNum)\"><v>\(stat.paymentMethod)</v></c>"
    ...
    private static func createGroupsSheet(event: Event) -> String {
        ...
        xml += "<row r=\"\(rowNum)\"><c r=\"A\(rowNum)\" s=\"1\"><v>BY TYPE</v></c></row>\n"
        rowNum += 1
        
        // Header row for categories
        xml += "<row r=\"\(rowNum)\">"
        xml += "<c r=\"A\(rowNum)\" s=\"1\"><v>Category</v></c>"
        xml += "<c r=\"B\(rowNum)\" s=\"1\"><v>Total Units</v></c>"
        xml += "<c r=\"C\(rowNum)\" s=\"1\"><v>Total Amount</v></c>"
        xml += "</row>\n"
        rowNum += 1
        ...
        for (_, stat) in sortedCategories {
            xml += "<row r=\"\(rowNum)\">"
            xml += "<c r=\"A\(rowNum)\"><v>\(xmlEscape(stat.category.name))</v></c>"
        ...
        xml += "<row r=\"\(rowNum)\"><c r=\"A\(rowNum)\" s=\"1\"><v>BY SUBGROUP</v></c></row>\n"
        rowNum += 1
        
        // Header row for subgroups
        xml += "<row r=\"\(rowNum)\">"
        xml += "<c r=\"A\(rowNum)\" s=\"1\"><v>Subgroup</v></c>"
        xml += "<c r=\"B\(rowNum)\" s=\"1\"><v>Total Units</v></c>"
        xml += "<c r=\"C\(rowNum)\" s=\"1\"><v>Total Amount</v></c>"
        xml += "</row>\n"
        rowNum += 1
        ...
        for (subgroup, stat) in sortedSubgroups {
            xml += "<row r=\"\(rowNum)\">"
            xml += "<c r=\"A\(rowNum)\"><v>\(xmlEscape(subgroup))</v></c>"
            xml += "<c r=\"B\(rowNum)\"><v>\(stat.units)</v></c>"
            xml += "<c r=\"C\(rowNum)\"><v>\(formatDecimalXML(stat.total))</v></c>"
            xml += "</row>\n"
            rowNum += 1
